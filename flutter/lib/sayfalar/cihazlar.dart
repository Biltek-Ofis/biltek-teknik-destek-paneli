...
class _CihazlarSayfasiState extends State<CihazlarSayfasi> {
      ...
      child: Consumer<MyNotifier>(
        builder: (context, MyNotifier myNotifier, child) {
          return Scaffold(
            ...
            appBar: aramaEtkin
                ? AppBar(flexibleSpace: aramaAppBar)
                : AppBar(
                    automaticallyImplyLeading: false,
                    title: Text(
                      arama.isNotEmpty
                          ? "\"$arama\" Arama Sonuçları"
                          : widget.seciliSayfa == "Anasayfa"
                          ? "Biltek Teknik Servis"
                          : widget.seciliSayfa,
                    ),
                    actions: [
                      IconButton(
            ...
            floatingActionButtonLocation: chatEnabled
                    ...
                    : RefreshIndicator(
                        ...
                        child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            Color? renkTemp = Islemler.yaziRengi(
  ...
  Future<void> _cihazlariYenile({bool sifirla = true}) async {
    if (sifirla) {
      setState(() {
        suankiIndex = 0;
      });
    }
    List<Cihaz> cihazlarTemp = await BiltekPost.of(widget.kullanici.auth)
        .cihazlariGetir(
...
Drawer biltekDrawer(
  ...
  EdgeInsetsGeometry titlePadding = EdgeInsets.symmetric(
    vertical: 0,
    horizontal: 8,
  );
  EdgeInsetsGeometry linkPadding = EdgeInsets.symmetric(
    vertical: 0,
    horizontal: 6,
  );
  return Drawer(
    child: ListView(
      ...
      children: [
        DrawerHeader(child: Image.asset(BiltekAssets.logo)),
        SizedBox(height: 5),
        SizedBox(
          width: MediaQuery.of(context).size.width,
          child: Container(
        ...
        if (!kullanici.musteri && (kullanici.yonetici || kullanici.sifreler))
          ListTile(
            contentPadding: linkPadding,
            title: const Text("Müşteri Şifreleri"),
            selected: seciliSayfa == "sifreler",
            onTap: () {
              Navigator.pop(context);
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) =>
                      Anasayfa(sayfa: "sifreler", kullanici: kullanici),
                ),
              );
            },
          ),
          ...
          ListTile(
            ...
            onTap: () {
              ...
              Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) =>
                      Anasayfa(sayfa: "malzeme_teslimi", kullanici: kullanici),
                ),
              );
            },
          ),
        ListTile(
          ...
          onTap: () {
            ...
            Navigator.of(context).pushAndRemoveUntil(
              MaterialPageRoute(
                builder: (context) =>
                    Anasayfa(sayfa: "cagri", kullanici: kullanici),
              ),
              (route) => false,
            );
          },
            ...
            onTap: seciliSayfa != "Lisanslar"
                ? () {
                    ...
                    Navigator.of(context).pushAndRemoveUntil(
                      MaterialPageRoute(
                        builder: (context) =>
                            LisansSayfasi(kullanici: kullanici),
                      ),
                      (route) => false,
                    );
                  }
            ...
            onTap: seciliSayfa != "Versiyonlar"
                ? () {
                    ...
                    Navigator.of(context).pushAndRemoveUntil(
                      MaterialPageRoute(
                        builder: (context) =>
                            VersiyonSayfasi(kullanici: kullanici),
                      ),
                      (route) => false,
                    );
                  }
      ...
      builder: (context) => BarcodeScanner(
        ...
        onScanned: (res) async {
          ...
          Future.delayed(Duration(seconds: 1), () async {
            ...
            if (res.isNotEmpty && res != "-1" && cihazNoCls.isValidCihaz(res)) {
              cihazNoCls.qrAc(
                qr: res,
                kullanici: kullanici,
                cihazlariYenile: cihazlariYenile,
