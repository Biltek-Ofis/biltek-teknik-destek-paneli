<script>
    $(document).ready(function () {
        $("#yapilanIslem<?= $index; ?>").on("keyup change", function () {
            var islemVal = $("#yapilanIslem<?= $index; ?>").val() + '';
            var miktarVal = $("#yapilanIslemMiktar<?= $index; ?>").val() + '';
            if (islemVal.length > 0 && (miktarVal == "" || miktarVal == "0")) {
                $("#yapilanIslemMiktar<?= $index; ?>").val("1");
            } else if (islemVal.length == 0 && (miktarVal == "" || miktarVal == "0" || miktarVal == "1")) {
...
echo '<tr id="yapilamIslemRow' . $index . '">
    ...
    <td>
        ...
        <div class="p-0 m-0 col">
            <input id="yapilanIslem' . $index . '" autocomplete="' . $this->Islemler_Model->rastgele_yazi() . '" name="islem' . $index . '" class="form-control" type="text" placeholder="İşlem" value="' . (isset($yapilanIslemArr["islem"]) ? $yapilanIslemArr["islem"] : "") . '"' . (OTO_TAMAMLAMA ? "" : " readonly") . '>
