var islemBilgileriHover = [];
var islemBilgileriOnaylandi = [];
function islemChange(i){
...
function listeyiGizle(i){
	if (!islemBilgileriHover[i - 1]) {
		$("#ucret_liste_" + i).hide();
	}
}
var yapilanIslemSayisi = 10;
$(document).ready(function () {
	for (let i = 1; i <= yapilanIslemSayisi; i++) {
		islemBilgileriHover.push(false);
		islemBilgileriOnaylandi.push(false);
	}
	for (let i = 1; i <= yapilanIslemSayisi; i++) {
		$("#yapilanIslem" + i).focus;
		$("#ucret_liste_" + i).hover(
			function () {
		...
		$("#yapilamIslemRow" + i).focusout(function () {
			//listeyiGizle(i);
			//console.log("Focus out liste "+i);
		});
		$("#yapilanIslem" + i).focus(function () {
			var yapilanIslem = $("#yapilanIslem" + i).val();
			if (!islemBilgileriOnaylandi[i - 1]) {
				stoklariGetir(yapilanIslem, i);
			}
			console.log("yapilanIslem focus "+i);
		});
		$("#yapilanIslem" + i).on("input", function () {
			islemChange(i);
			var yapilanIslem = $("#yapilanIslem" + i).val();
			stoklariGetir(yapilanIslem, i);
		});
		$("#yapilanIslem" + i).on("change", function () {
			islemChange(i);
		});
		$(
			"#yapilanIslemMiktar" +
		...
		$("#yapilanIslemMaliyet" + i).on("change keyup", function () {
			var toplamMaliyet = 0;
			for (let i = 1; i <= 10; i++) {
				var maliyet = $("#yapilanIslemMaliyet" + i).val();
				toplamMaliyet = toplamMaliyet + (maliyet.length > 0 ? parseFloat(maliyet) : 0);
			}
			$("#yapilanIslemToplamMaliyet").html(
				toplamMaliyet > 0 ? parseFloat(toplamMaliyet).toFixed(2) + " TL" : ""
			);
			karHesapla();
		});
	}
...
function karHesapla() {
	var toplamMaliyet = $("#yapilanIslemToplamMaliyet").html();
	if (toplamMaliyet.length > 0) {
		toplamMaliyet = parseFloat(toplamMaliyet.replace(" TL", ""));
	} else {
		toplamMaliyet = 0;
	}
	var genelToplam = $("#yapilanIslemToplam").html();
	if (genelToplam.length > 0) {
		genelToplam = parseFloat(genelToplam.replace(" TL", ""));
	} else {
		genelToplam = 0;
	}
	var kar = genelToplam - toplamMaliyet;
	$("#yapilanIslemToplamKar").html(
		parseFloat(kar).toFixed(2) + " TL"
	);
}

function stoklariGetir(yapilanIslem, i) {
	if (yapilanIslem.length > 1) {
		$.post(base_url + "/js/ucretler", { ara: yapilanIslem }, function (data) {
			var jsonData = JSON.parse(data);
			if (Object.keys(jsonData).length > 0) {
				$("#ucret_liste_" + i).html("");
				$.each(JSON.parse(data), function (index, value) {
					var oge = $(
						'<li class="active"><a class="dropdown-item" href="javascript:void(0);" id="ucret_liste_oge_' +
							i +
							"_" +
							index +
							'" role="option">' +
							value.isim + " - " + value.ucret + " TL"+
							"</a></li>"
					);
					$("#ucret_liste_" + i).append(oge);
					$("#ucret_liste_oge_" + i + "_" + index).on("click", function (e) {
						e.preventDefault();
    					e.stopPropagation(); 
...
function stokVerileri(value, i) {
	//$("#stokKodText" + i).html(value.id ? value.id : "Yok");
	//$("#yapilanIslemStokKod" + i).val(value.id ? value.id : "");
	$("#yapilanIslemMiktar" + i).val(1);
	$("#yapilanIslem" + i).val(value.isim ? value.isim : "");
	$("#yapilanIslemFiyat" + i).val(
		parseFloat(value.ucret ? value.ucret : 0).toFixed(2)
	);
	islemBilgileriOnaylandi[i - 1] = true;
	$("#ucret_liste_" + i).hide();
	islemHesapla(i);
}
function _(abc) {
...
function medyaSil(cihaz_id, id, tamamlama_func) {
	$.post(base_url + "/cihaz/medyaSil/" + cihaz_id + "/" + id + "/post", {})
		.done(function (msg) {
			try {
				...
				alert("Medya Silinemedi! Lütfen daha sonra tekrar deneyin");
				console.log("Medya Silinemedi: " + msg);
